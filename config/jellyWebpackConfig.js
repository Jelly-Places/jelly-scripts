...
const getCSSModuleLocalIdent = function jellyGetLocalIdent(
  context,
  localIdentName,
  localName,
  options
  ...
  const fileNameOrFolder = context.resourcePath.match(/(index|index\.module)\.(css|scss|sass)$/)
    ? '[folder]'
    : '[name]';
...
const checkConfigStructure = config => {
  ...
  const hasRules =
    config && config.module && config.module.rules && config.module.rules.length === 3;
  const hasOneOf =
    hasRules && config.module.rules[2].oneOf && config.module.rules[2].oneOf.length === 8;
  const hasCssLoader =
    hasOneOf &&
...
const applyJellyConfigs = (config, isEnvProduction) => {
